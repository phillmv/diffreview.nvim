...
local function render_scope(scope)
  local left = scope.left.kind == "commit"
      and scope.left.oid:sub(1, 12)
      or scope.left.kind
  local right = scope.right.kind == "working_tree"
      and "WORKING TREE"
      or scope.right.oid:sub(1, 12)
  return left .. ".." .. right
end
...
local function render_markdown(review, comments)
  local lines = {
    ...
    "- Range: `" .. render_scope(review.scope) .. "`",
